...
- Stateless vault mode produces no audit log — constitutional review fires in stateless mode but outcomes are not persisted because vault writes are disabled. If a review triggers a revision or refusal, the intervention is applied to the response but no record exists in logs/safety_reviews/. This creates an audit gap for stateless conversations.
