...
# ── Bug report (server-side GitHub Issues proxy) ─────────────────────
# The GitHub token is stored in keyring, not in the UI build. This
# endpoint replaces the previous client-side GitHub API call in
# ember-2-ui (which embedded the token in VITE_GITHUB_TOKEN, making
# it extractable from the built JS). The token should be treated as
...
@limiter.limit("5/minute")
def bug_report_endpoint(request: Request, body: BugReportRequest):
    """Create a GitHub issue via server-side proxy.

    The GitHub personal access token is read from the OS credential
    store (keyring service "ember-2-github", username "token"). If no
    token is configured, returns 503. The UI should not hold any GitHub
    credentials — all issue creation flows through this endpoint.
    """
    ...
    try:
        import keyring
        token = keyring.get_password("ember-2-github", "token")
    except Exception:
        token = None
    ...
    if not token:
        raise HTTPException(
            status_code=503,
            detail="GitHub token not configured. Run: python scripts/set_github_token.py",
        )

    import httpx
    try:
        resp = httpx.post(
            "https://api.github.com/repos/niansahc/ember-2/issues",
            json={
                "title": body.title.strip(),
                "body": body.body.strip() if body.body else "",
            },
            headers={
        ...
        raise HTTPException(status_code=500, detail=f"Bug report failed: {exc}")
