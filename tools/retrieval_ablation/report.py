...
CAVEATS: tuple[str, ...] = (
    "1. `tiering does not help` is NOT reachable from the production vault.",
    "   The access term is dead: ContextItem.id is a file path, never the",
    "   vectors primary key, so retrieval_count is non-zero on 1 record of",
    "   16,986. _recency_score returns 0.0 for every ingested record because",
    "   their created_at is a float epoch its date parser cannot read. The",
    "   live corpus is 99.92% cold with zero warm records, and heat's only",
    "   live term is `importance`, a six-value TYPE lookup -- so in production",
    "   tiering and typing are not independent. Any `tiering does not help`",
    "   reading from this run is a statement about that defect, not about the",
    "   design. This eval stipulates a real tier mix on synthetic fixtures and",
    "   must be read as measuring the MECHANISM, not the deployed system.",
    "",
